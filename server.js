...
app.post('/webhook/whatsapp', async (req, res) => {
  try {
    ...
    if (produits.length > 0) {
      ...
      await pool.query(
        'INSERT INTO orders (customer_phone, items, status) VALUES ($1, $2, $3)',
        [expediteur, JSON.stringify(produits), 'nouveau']
      );

      // Mettre à jour client
      await pool.query(`
        INSERT INTO clients (phone, total_orders) VALUES ($1, 1)
        ON CONFLICT (phone) DO UPDATE SET total_orders = clients.total_orders + 1
      `, [expediteur]);
...
app.post('/api/products', authMiddleware, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO products (name, price, stock, unit) VALUES ($1,$2,$3,$4) RETURNING *',
      [name, price, stock, unit]
    );
    if (parseInt(stock) < 5) {
      console.log(`⚠️ ALERTE Stock faible: ${name} (${stock} restants)`);
    }
...
app.put('/api/products/:id', authMiddleware, async (req, res) => {
  const { name, price, stock, unit } = req.body;
  const result = await pool.query(
    'UPDATE products SET name=$1, price=$2, stock=$3, unit=$4 WHERE id=$5 RETURNING *',
...
app.post('/api/invoices', authMiddleware, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO invoices (order_id, number, amount) VALUES ($1,$2,$3) RETURNING *',
      [order_id, number, amount]
    );
    res.json(result.rows[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
  ...
  try {
    ...
    res.json({
      ...
      kpis: {
        ...
        impayes: parseInt(impayes.rows[0].count),
        clients: parseInt(clients.rows[0].count)
      },
      alertes: {
        stock_faible: produits_faible.rows
      },
...
app.get('/api/roi', authMiddleware, async (req, res) => {
  ...
  res.json({
    ...
    commandes: parseInt(commandes.rows[0].count),
    revenus_fcfa: parseFloat(revenus.rows[0].total),
    cout_abonnement: 10000,
    roi: `${Math.round((parseFloat(revenus.rows[0].total) / 10000) * 100)}%`
  });
});

// ===== HEALTH CHECK =====
app.get('/health', (req, res) => {
  res.json({ status: 'ok', app: 'MarchandPro', version: '1.0.0' });
});

app.get('/', (req, res) => {
  res.json({ message: 'Bienvenue sur MarchandPro API 🇸🇳', status: 'running' });
});

// DEMARRAGE
initDB().then(() => {
  app.listen(3000, () => {
    console.log('🚀 MarchandPro démarré sur port 3000');
  });
}).catch(err => {
